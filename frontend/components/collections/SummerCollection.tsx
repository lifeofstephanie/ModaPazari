    ...
    <section className="mt-5 md:mt-20">
      <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-b border-border pb-8">
        <div>
          ...
          <h2 className="text-4xl md:text-5xl font-serif tracking-tight mt-2 italic text-accent font-bold">
            Summer Collection
          </h2>
        </div>
        <p className="text-muted text-sm max-w-xs mt-4 md:mt-0 font-light italic">
        ...
        {filteredClothes.map((item) => (
          <Link href={`/shop/${item.id}`} key={item.id} className="group">
            <div className="relative">
              <div className="relative aspect-[3/4] overflow-hidden rounded-[40px] border-[12px] border-border">
                <img
                  src={item.img}
                  alt={item.name}
                  loading="lazy"
                ...
                />
                <div className="absolute bottom-6 right-6 w-16 h-16 bg-card rounded-full flex items-center justify-center shadow-lg transform transition-transform group-hover:rotate-45">
                  <span className="text-[10px] font-bold tracking-tighter text-center leading-none uppercase">
                    Moda <br /> Pazari
                  </span>
                  <div className="absolute -top-1 -right-1 bg-accent-solid text-white rounded-full p-1">
                    <svg
                      width="12"
                      height="12"
                      viewBox="0 0 24 24"
              ...
              <div className="mt-6 text-center">
                <h3 className="text-xl font-serif text-accent italic">
                  {item.name}
                </h3>
                <p className="text-accent font-bold mt-1 tracking-widest text-sm">
      ...
      <div className="flex justify-end items-end mt-5">
        <Link href={"/"}>
          <p className="hover:underline text-right text-accent">
            See More...
          </p>
        </Link>
      </div>
